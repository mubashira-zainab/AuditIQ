...
tool — **and it now serves the website too.** One server, one port, one URL.
No more running a separate frontend server.

The `web/` folder holds the frontend (`index.html`, `style.css`, `app.js`,
logo files). FastAPI serves it directly alongside the API, so visiting the
server's root URL loads the whole app, and the frontend's fetch calls go to
`/api/...` on that same origin automatically -- no CORS, no separate ports,
no "Backend URL" field to fill in unless you deliberately split them apart
again later.

## Project layout

```
web/                   The website: index.html, style.css, app.js, logo files
app/
  main.py              FastAPI app factory: wires routers, CORS, error handling,
                        and mounts web/ so one server serves everything
...
Open **http://localhost:8000** — that's the whole app: upload a file, run
analysis, listen to the briefing, chat, all from this one URL.

Interactive API docs (if you want to test endpoints directly): `http://localhost:8000/docs`
